...
export class CompanyNotificationsController {
  ...
  static async listNotifications(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const whereConditions: string[] = ['n.user_id = ?', "n.user_type = 'company'"];
    ...
    try {
      ...
      const result = await queryOne<{ count: number }>(
        `SELECT COUNT(*) as count
         FROM notifications
         WHERE user_id = ? AND user_type = 'company' AND is_read = FALSE`,
        [userId]
      );
  ...
  static async markAsRead(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const notification = await queryOne<any>(
        `SELECT id FROM notifications WHERE id = ? AND user_id = ? AND user_type = 'company'`,
        [id, userId]
      );
      ...
      res.status(200).json(
        createSuccessResponse({ message: 'Notificação marcada como lida' })
      );
    } catch (error) {
      next(error);
    }
  ...
  static async markAllAsRead(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.status(200).json(
        createSuccessResponse({ message: 'Todas as notificações foram marcadas como lidas' })
      );
    } catch (error) {
      next(error);
    }
  ...
  static async deleteNotification(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      res.status(200).json(
        createSuccessResponse({ message: 'Notificação removida com sucesso' })
      );
    } catch (error) {
      next(error);
    }
